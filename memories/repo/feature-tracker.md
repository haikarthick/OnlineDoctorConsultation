...
### ✅ Species Name i18n Fix
- **Logged:** 2026-07-23
- **Status:** done
- **Description:** master_species had no label_key mechanism (unlike marketplace categories/conditions) — species names (dog/buffalo/cattle/etc.) never translated into any of the 6 locales anywhere in the app, regardless of user language. Reported via screenshot: PublicMarketplace species filter showing "Buffalo" untranslated in Tamil locale. Fix: migration 024 adds master_species.label_key, backfilled with deterministic speciesNames.<code> keys; new speciesLabel(code, t) resolver in MasterDataContext (typed against real i18next TFunction, not a hand-rolled shape — that distinction matters, see lessons.md); 51-key speciesNames block added to all 6 locale files; propagated to 31 files across Marketplace/PublicMarketplace/Animals/HospitalWorkflow/VaccinationPassport/MedicalRecords/pharmacy/doctor/petowner/admin/analytics pages. Fixed a NetworkMemberships.test.tsx regression the change surfaced. TypeScript clean both sides, full frontend test suite green (30/30).


### ✅ Species Per-Locale Admin Labels (self-serve, no deploy needed)
- **Logged:** 2026-07-23
- **Status:** done
- **Description:** Follow-up to the species i18n fix above. Question raised: does a species an admin adds later auto-translate? No — the admin form had no labelKey field, and labelKey only resolves if a developer adds a matching key to all 6 static locale files and redeploys. Fix: migration 025 adds nullable label_hi/label_kn/label_ml/label_ta/label_te directly on master_species; admin species form now has 5 optional per-locale text inputs; speciesLabel() in MasterDataContext checks these row-level overrides for the active UI locale first (i18n.language read inside MasterDataProvider itself, so the exposed speciesLabel(code, t) signature never changed and none of the 31 existing call sites needed touching), falling back to the old labelKey/static-key path only when a row has no override — the two mechanisms coexist without conflict. TypeScript clean both sides, full frontend test suite green (30/30).
