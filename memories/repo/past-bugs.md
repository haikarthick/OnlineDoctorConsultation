...
### DEPLOY-012 — uuid_generate_v4() Recurred in backend/migrations/*.sql, Silently Broken for Unknown Time, Then Blocked a Deploy (2026-07-21)
- **Symptom:** Render deploy for `vetcare-dev` failed outright: "Exited with status 1 while running your code" during "Running database migrations". Log showed `002_missed_by.sql` and `003_cancellation_wallet_payment.sql` both failing with `function uuid_generate_v4() does not exist`.
- **Root Cause:** Same root cause as DEPLOY-006 — Render's managed PostgreSQL doesn't allow `CREATE EXTENSION "uuid-ossp"` (restricted role), so `uuid_generate_v4()` is never available. DEPLOY-006's fix ("573 replacements") explicitly covered `docker/init.sql`, seed files, `database.ts`, and "all migration **.ts** files" — but NOT the `.sql` files under `backend/migrations/`. Four files there (`002_missed_by.sql`, `002_staff_workflow.sql`, `003_cancellation_wallet_payment.sql`, `004_vet_certificates.sql`) used `uuid_generate_v4()` and were never caught by that sweep or by any automated check.
- **Why this had been invisible:** Both the in-process migration runner (`database.ts`'s `runSQLMigrations()`) and the CLI runner (`migrate.ts`, called by `render-start.sh` before the server starts) used to catch a failed migration, log a warning, and continue — by design, so a deploy never failed outright. This meant `002_missed_by.sql` / `003_cancellation_wallet_payment.sql` could have been silently failing on every single deploy for as long as they existed, with their actual schema changes (a `bookings.missed_by` column, a `bookings.cancelled_by`/`cancelled_at` pair, the `wallets`/`wallet_transactions` tables, several `system_settings` rows) never actually applied to this environment — invisible until a separate hardening change (see MIGRATIONS_FAIL_FAST below) made migration failures fail the deploy instead of silently continuing, which is what surfaced this.
- **Fix:** Replaced all `uuid_generate_v4()` → `gen_random_uuid()` in the 4 migration files above, plus a stray untracked root-level script `backend/migrate_medical_records.js` (not part of any deploy path, but same bug class).
- **Process fix (the actual point of this entry):** A memory-file rule alone was not enough — this exact pattern was already documented (DEPLOY-006, LESSON-001) and still recurred, because nothing *automated* checked for it. Added a real check: `backend/scripts/schema-check.js` now scans every `.sql` file under `docker/` and `backend/migrations/` for `uuid_generate_v4(` and `CREATE EXTENSION.*uuid-ossp`, and fails the pre-push hook (`schema-check.js` is one of the 4 checks `pre-deploy.js` runs) if found. This is NOT optional/manual — it runs on every `git push` automatically. Verified it actually catches the pattern (planted a test file with `uuid_generate_v4()`, confirmed it fails) before relying on it.
- **Rule: this exact bug class (banned SQL patterns) is now enforced by `backend/scripts/schema-check.js` on every push — but if you add a NEW class of "works locally, breaks on Render's restricted DB role" pattern, add a check for it there too rather than only writing a memory note.** A memory note didn't stop this from recurring once already.
- **Related:** See "DEPLOY-013" (below) for the accompanying change that made migration failures actually fail the deploy instead of silently continuing — that's what surfaced this bug, and very nearly caused a real deploy outage in the process (see that entry for the nuance around distinguishing a genuine SQL failure from a transient Render cold-start connectivity issue).

---

### DEPLOY-013 — Made Migrations Fail-Fast on Deploy Without DB Access to Verify Against — Immediately Hit DEPLOY-012 (2026-07-21)
- **Context:** A code-quality audit asked to "make migrations the sole schema authority... and fail deployment on an unapplied or failed migration." The existing tolerate-and-continue behavior was a *deliberate* design (see index.ts: server binds its HTTP port before touching the DB specifically so Render's health check always passes on free-tier cold starts, and explicitly never crashes the already-running server over a DB issue). No local Postgres/Docker was available in the working environment to verify the 20 existing migration files would even pass a fail-fast check before shipping the change.
- **What happened:** User was told explicitly about this risk and chose to proceed anyway ("make it fail-fast anyway"). The very next real deploy hit exactly the un-verifiable failure mode: DEPLOY-012 above. Production deploy was blocked.
- **Why it wasn't worse:** The fix was scoped narrowly on purpose — only `render-start.sh`'s separate, pre-server-start `migrate.js` step was made to abort the deploy; the in-process, already-resilient server startup (`database.ts`'s own migration runner) was deliberately left non-fatal. So the failure showed up as a clean, clearly-diagnosable "deploy failed, here's exactly which 2 migrations and why" in the Render dashboard — not a crashed live server, not cascading errors for connected users, not a silent partial-migration state shipped to production.
- **Escape hatch that was built in for exactly this scenario:** `MIGRATIONS_FAIL_FAST=false` as a Render env var — reverts to the old tolerant behavior without a code rollback, for exactly the "this change turns out to be wrong against real prod state" case. (Wasn't needed here — the actual migrations were fixed and pushed instead, since the fix was fast and safe.)
- **Rule: when a change's safety depends on production state you cannot inspect (no DB access, no staging environment), either (a) get explicit, informed sign-off before shipping — which happened here — AND (b) always build a fast, no-redeploy-needed revert path (env var, feature flag) alongside it, not just the forward change. Assume the unverifiable risk WILL materialize, because it did, on the very next deploy.**
